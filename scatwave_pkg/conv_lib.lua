...
function conv_lib.periodize_along_k(h,k,l)
   ...
   local reshaped_h=torch.reshape(h,new_dim)

 local  summed_h=torch.reshape(torch.sum(reshaped_h,k),final_dim)
return summed_h
end
...
fftw.execute(plan)
  ...
  -- output_first_half   =output:narrow(k,1,torch.floor(x:size(k)/2))
    --  output_second_half=output:narrow(k,torch.floor(x:size(k)/2)+1,torch.floor(x:size(k)/2))

   --output_second_half:indexCopy(k,torch.linspace(1,x:size(k),x:size(k)):long(),output_first_half)
