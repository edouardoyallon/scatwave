...
function wrapper_fft.my_2D_fft_complex_batch_inplace(x,k,backward,out)
   
end

function wrapper_fft.my_2D_fft_real_batch_inplace(x,k)
   
end

function wrapper_fft.my_2D_ifft_complex_to_real_batch_inplace(x,mini_batch,out)
-- Defines a 2D convolution that batches until the k-th dimension
   local dims = ffi.new('int[?]',2)
   dims[0] = x:size(k)
   dims[1] = x:size(k+1)
