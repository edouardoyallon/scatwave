...
function conv_lib.my_convolution_2d(x,filt,ds,mini_batch,my_fft,scat,res)
   filt=filt[res+1]
...
--   assert(tools.are_equal_dimension(x,filt),'The signal should be of the same size')
   ...
   return my_fft.my_2D_fft_complex_batch(yf,1+mini_batch,1,my_fft.lookuptable[res])
end
