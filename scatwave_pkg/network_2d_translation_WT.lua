...
local network = torch.class('network_WT')
...
function network:__init(J,U0_dim)
   ...
   size_S[#U0_dim-1]=1 + J*8 --+ J*(J-1)/2*8*8 -- number of layers
      size_S[#size_S-1] = 1+torch.floor((U0_dim[#U0_dim]-1)/2^0)
   size_S[#size_S] = 1+torch.floor((U0_dim[#U0_dim]-1)/2^0)
